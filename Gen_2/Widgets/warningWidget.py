...
class EmptyStart(QMainWindow):
    ...
    def load_ui(self):
        """Load the UI file"""
        try:
            ui_file = QFile(self.directoryParent+r"\UI\Empty Start.ui")  # Change to your .ui file name
...
class EmptyLeave(QMainWindow):
    ...
    def load_ui(self):
        """Load the UI file"""
        try:
            ui_file = QFile(self.directoryParent+r"\UI\Warning Chat.ui")  # Change to your .ui file name
            if not ui_file.open(QFile.ReadOnly):
                print(f"Cannot open UI file: {ui_file.errorString()}")
                return
